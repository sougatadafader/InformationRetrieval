...
while( $front <= $rear )
{
	...
	foreach( $main_links as $link )
	{
		...
		$anchor_text = strtolower( $link->plaintext );
		...
		$flag_proceed = false;
		/*if( strpos( $anchor_text,'rain' ) == 0 )
		{
			$flag_proceed = true;
		}*/
		$string_start = substr( $anchor_text,0,4 );
		if( $string_start == 'rain' )
		{
			$flag_proceed = true;
		}
		$anchor_text_split = explode( ' ',$anchor_text );
		if( in_array( 'rain',$anchor_text_split ) )
		{
			$flag_proceed = true;
		}
		if( !isset( $links_index[$link_href] ) && empty( $links_index[$link_href] ) && $flag_proceed == true )
		{
	...
	file_put_contents( 'saved_rain/'.$file_title.'.html',$url_src );
	$obj = array(
		'heading'	=>	$heading,
		'depth'		=>	$depth,
...
//print_r( $links_queue );
file_put_contents( 'links_rain.txt',$links_str );
